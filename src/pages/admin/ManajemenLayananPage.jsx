    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50, overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ padding: '10px 16px 0' }}>
        <button
          ...
          style={{
            ...
            boxShadow: '0 4px 14px rgba(110,46,120,0.22)',
            transition: 'all 0.2s ease',
          }}
          onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(110,46,120,0.30)'; }}
          onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 14px rgba(110,46,120,0.22)'; }}
        >
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <span style={{
                ...
                background: C.primary, color: 'white', fontFamily: 'Poppins', fontSize: 9, fontWeight: 600,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              }}>
                {activeFilterCount}
              </span>
      ...
      <Modal visible={filterOpen} onClose={() => setFilterOpen(false)} title="Filter Layanan">
        <div style={{ padding: '16px 18px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 18 }}>
            <button type="button" onClick={() => { setFilter('all'); setStatusFilter('all'); setSortBy('name_asc'); }} style={{
              flex: 1, height: 38, borderRadius: 10, border: `1.5px solid ${C.n200}`,
              background: C.n50, fontFamily: 'Poppins', fontSize: 12, fontWeight: 600, color: C.n700, cursor: 'pointer',
            }}>Reset</button>
            <button type="button" onClick={() => setFilterOpen(false)} style={{
              flex: 1, height: 38, borderRadius: 10, border: 'none',
              background: C.primary, fontFamily: 'Poppins', fontSize: 12, fontWeight: 600, color: 'white', cursor: 'pointer',
            }}>Terapkan</button>
          </div>
        </div>
      </Modal>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 24px' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '50%', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'Poppins', fontSize: 12, color: C.n500 }}>Tap + untuk menambahkan layanan</span>
          </div>
        ) : (
            ...
            {filtered.map((s) => {
              ...
              return (
                <div key={s.id} style={{
                  ...
                  boxShadow: SHADOW.md, opacity: s.active === false ? 0.55 : 1,
                  transition: 'all 0.2s ease',
      ...
      <Modal visible={modalAdd} onClose={() => setModalAdd(false)} title={editingId ? "Edit Layanan" : "Tambah Layanan"}>
        <div style={{ padding: '4px 4px 0' }}>
          ...
          {/* Express toggle */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, padding: '10px 14px', background: C.n50, borderRadius: 12 }}>
            <div>
            ...
            <button type="button" onClick={() => setForm((f) => ({ ...f, expressEligible: !f.expressEligible }))} style={{
              ...
              <div style={{
                ...
                transition: 'left 0.2s', boxShadow: SHADOW.smSoft,
              }} />
            </button>
          </div>
