...
function PctBadge({ pct }) {
  ...
  const color = pct >= 100 ? '#059669' : pct >= 80 ? '#10B981' : pct >= 50 ? '#F59E0B' : '#EF4444';
  const bg    = pct >= 100 ? '#DCFCE7' : pct >= 80 ? '#D1FAE5' : pct >= 50 ? '#FEF3C7' : '#FEE2E2';
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', background: bg, padding: '2px 10px', borderRadius: 999 }}>
      <span style={{ fontFamily: 'Poppins', fontSize: 11, fontWeight: 800, color }}>{pct}% target</span>
    </div>
  );
}
...
export default function AdminPeriodClosePage({ goBack }) {
  ...
  const pctColor = (p) => p >= 100 ? '#059669' : p >= 80 ? '#10B981' : p >= 50 ? '#F59E0B' : '#EF4444';
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 80px' }}>
        ...
        {/* Outlet selector */}
        <div style={{ background: 'white', borderRadius: 14, padding: '12px 14px', marginBottom: 16, boxShadow: '0 2px 8px rgba(15,23,42,0.06)' }}>
          <Select
            label="Outlet"
            value={selOutlet}
        ...
        {/* Current period card */}
        {loading ? (
          <div style={{ textAlign: 'center', padding: '32px 0', fontFamily: 'Poppins', fontSize: 13, color: C.n500 }}>Memuat...</div>
        ) : period && (
          <div style={{
            background: 'white', borderRadius: 16, padding: '16px', marginBottom: 16,
            boxShadow: '0 2px 12px rgba(15,23,42,0.09)',
            border: `2px solid ${period.alreadyClosed ? '#10B981' : period.isClosing ? '#F59E0B' : C.n200}`,
          }}>
            {/* Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
              ...
              <div>
                {period.alreadyClosed ? (
                  <span style={{ background: '#DCFCE7', color: '#059669', fontFamily: 'Poppins', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 999 }}>
                    ✅ Sudah Ditutup
                  </span>
                ) : period.isClosing ? (
                  <span style={{ background: '#FEF3C7', color: '#92400E', fontFamily: 'Poppins', fontSize: 11, fontWeight: 700, padding: '4px 12px', borderRadius: 999 }}>
                    ⚠️ {period.daysLeft} hari lagi
                  </span>
                ) : (
                  <span style={{ background: C.n100, color: C.n600, fontFamily: 'Poppins', fontSize: 11, fontWeight: 600, padding: '4px 12px', borderRadius: 999 }}>
            ...
            {!period.alreadyClosed && period.isClosing && (
              <div style={{ background: '#FEF3C7', borderRadius: 10, padding: '10px 12px', marginBottom: 14 }}>
                ...
                <div style={{ fontFamily: 'Poppins', fontSize: 11, color: '#78350F' }}>
            ...
            {period.alreadyClosed && period.closedAt && (
              <div style={{ background: '#DCFCE7', borderRadius: 10, padding: '8px 12px', marginBottom: 14 }}>
                <div style={{ fontFamily: 'Poppins', fontSize: 11, color: '#065F46' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
              {[
                ...
                { label: 'SELESAI', value: period.stats.totalSelesai, color: '#7C3AED' },
              ].map(s => (
                <div key={s.label} style={{ background: C.n50, borderRadius: 10, padding: '10px 12px' }}>
                  <div style={{ fontFamily: 'Poppins', fontSize: 9, fontWeight: 700, color: C.n500, letterSpacing: 0.5 }}>{s.label}</div>
                  <div style={{ fontFamily: 'Poppins', fontSize: 15, fontWeight: 800, color: s.color, marginTop: 2 }}>{s.value}</div>
                </div>
              ))}
            </div>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontFamily: 'Poppins', fontSize: 13, fontWeight: 700, color: C.n900, marginBottom: 10 }}>
            📂 Riwayat Tutup Buku — {outletName}
          </div>

          {histLoading ? (
            <div style={{ textAlign: 'center', padding: '24px 0', fontFamily: 'Poppins', fontSize: 12, color: C.n500 }}>Memuat riwayat...</div>
          ) : history.length === 0 ? (
            <div style={{ background: 'white', borderRadius: 14, padding: '24px', textAlign: 'center', boxShadow: '0 1px 6px rgba(15,23,42,0.06)' }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>📂</div>
              <div style={{ fontFamily: 'Poppins', fontSize: 12, color: C.n500 }}>Belum ada riwayat tutup buku untuk outlet ini.</div>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {history.map(row => {
                const color = row.pct != null ? pctColor(row.pct) : C.n600;
                const piutang = Math.max(0, row.totalOmset - row.totalPelunasan);
                return (
                  <div key={row.id} style={{ background: 'white', borderRadius: 14, padding: '14px 16px', boxShadow: '0 1px 8px rgba(15,23,42,0.06)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: 80, background: C.n50, borderRadius: 8, padding: '6px 10px' }}>
                        <div style={{ fontFamily: 'Poppins', fontSize: 9, color: C.n500, fontWeight: 700 }}>OMSET</div>
                        <div style={{ fontFamily: 'Poppins', fontSize: 12, fontWeight: 800, color: C.primary }}>{rp(row.totalOmset)}</div>
                      </div>
                      <div style={{ flex: 1, minWidth: 80, background: C.n50, borderRadius: 8, padding: '6px 10px' }}>
                        <div style={{ fontFamily: 'Poppins', fontSize: 9, color: C.n500, fontWeight: 700 }}>PELUNASAN</div>
                        <div style={{ fontFamily: 'Poppins', fontSize: 12, fontWeight: 800, color: C.success }}>{rp(row.totalPelunasan)}</div>
                      </div>
                      <div style={{ flex: 1, minWidth: 80, background: C.n50, borderRadius: 8, padding: '6px 10px' }}>
                        <div style={{ fontFamily: 'Poppins', fontSize: 9, color: C.n500, fontWeight: 700 }}>NOTA</div>
                        <div style={{ fontFamily: 'Poppins', fontSize: 12, fontWeight: 800, color: C.info }}>{row.totalTransaksi}</div>
                      </div>
                    </div>

                    {row.targetAmount != null && (
                      <>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'Poppins', fontSize: 10, color: C.n500, marginBottom: 3 }}>
                    ...
                    {piutang > 0 && (
                      <div style={{ marginTop: 8, fontFamily: 'Poppins', fontSize: 11, color: '#92400E', background: '#FEF3C7', borderRadius: 8, padding: '5px 10px' }}>
                        ⚠️ Piutang belum terbayar: {rp(piutang)}
                      </div>
                    )}

                    <div style={{ marginTop: 8, fontFamily: 'Poppins', fontSize: 10, color: C.n400 }}>
                      Ditutup oleh {row.closedByName || '—'} · {fmtDateTime(row.closedAt)}
                    </div>
                    {row.notes && (
                      <div style={{ marginTop: 4, fontFamily: 'Poppins', fontSize: 10, color: '#92400E' }}>📝 {row.notes}</div>
                    )}
                  </div>
                );
              })}
      ...
      <Modal isOpen={closeModal} onClose={() => setCloseModal(false)} title={`📒 Tutup Buku ${period?.periodLabel || ''}`}>
        <div style={{ fontFamily: 'Poppins', fontSize: 12, color: C.n600, marginBottom: 12, lineHeight: 1.6 }}>
        ...
        {period?.stats && (
          <div style={{ background: C.n50, borderRadius: 12, padding: '12px 14px', marginBottom: 16 }}>
            <div style={{ fontFamily: 'Poppins', fontSize: 11, fontWeight: 700, color: C.n700, marginBottom: 8 }}>Ringkasan periode ini:</div>
            {[
