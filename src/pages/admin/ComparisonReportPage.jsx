...
const Delta = ({ value, reverseGood = false }) => {
  ...
  const color = value === 0 ? C.n500 : good ? C.success : C.danger;
  return (
    <span style={{ ...F, fontSize: 11, fontWeight: 700, color, marginLeft: 4 }}>
...
const MetricCard = ({ label, currentVal, prevVal, delta, format = 'number', color = C.primary }) => {
  ...
  return (
    <div style={{ background: C.white, borderRadius: 14, padding: '14px 16px', boxShadow: '0 2px 8px rgba(15,23,42,0.06)' }}>
      <div style={{ ...F, fontSize: 10, color: C.n500, fontWeight: 600, letterSpacing: 0.3, marginBottom: 6 }}>{label}</div>
      <div style={{ ...F, fontSize: 20, fontWeight: 800, color, lineHeight: 1.1 }}>{fmt(currentVal)}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 4 }}>
        <span style={{ ...F, fontSize: 10, color: C.n500 }}>vs {fmt(prevVal)}</span>
        <Delta value={delta} />
      </div>
    </div>
  );
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px 24px' }}>
        ...
        <div style={{ background: C.white, borderRadius: 14, padding: 14, marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.06)' }}>
          <div style={{ ...F, fontSize: 12, fontWeight: 700, color: C.n700, marginBottom: 8 }}>Periode A (Saat Ini)</div>
          ...
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.n100}` }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ ...F, fontSize: 12, fontWeight: 700, color: C.n700 }}>Periode B (Pembanding)</div>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                <input type="checkbox" checked={customCompare} onChange={e => setCustomCompare(e.target.checked)} />
                <span style={{ ...F, fontSize: 11, color: C.n600 }}>Custom</span>
              </label>
            </div>
            {customCompare ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
              <div style={{ background: `${C.primary}12`, borderRadius: 10, padding: '8px 12px', borderLeft: `3px solid ${C.primary}` }}>
                <div style={{ ...F, fontSize: 9, fontWeight: 700, color: C.primary, letterSpacing: 0.5 }}>PERIODE A</div>
                <div style={{ ...F, fontSize: 11, color: C.n700, marginTop: 2 }}>{data.periodA.start} → {data.periodA.end}</div>
              </div>
              <div style={{ background: `${C.n400}12`, borderRadius: 10, padding: '8px 12px', borderLeft: `3px solid ${C.n400}` }}>
                <div style={{ ...F, fontSize: 9, fontWeight: 700, color: C.n600, letterSpacing: 0.5 }}>PERIODE B</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              ...
              <MetricCard label="CUSTOMER UNIK" currentVal={cur.uniqueCustomers} prevVal={prev.uniqueCustomers} delta={changes.uniqueCustomers} color="#8B5CF6" />
            </div>

            {/* Avg per transaction */}
            <div style={{ background: C.white, borderRadius: 14, padding: '12px 16px', marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.06)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ ...F, fontSize: 10, color: C.n500, fontWeight: 600 }}>RATA-RATA PER TRANSAKSI</div>
                  <div style={{ ...F, fontSize: 18, fontWeight: 800, color: C.n900, marginTop: 2 }}>{rp(cur.avgPerTx)}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ ...F, fontSize: 10, color: C.n500 }}>Periode B</div>
                  <div style={{ ...F, fontSize: 14, fontWeight: 600, color: C.n600 }}>{rp(prev.avgPerTx)}</div>
            ...
            <div style={{ background: C.white, borderRadius: 14, padding: '14px 16px', marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ ...F, fontSize: 13, fontWeight: 700, color: C.n900 }}>Tren Perbandingan</div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {[{ key: 'revenue', label: 'Omset' }, { key: 'txCount', label: 'Transaksi' }].map(m => (
                    <button key={m.key} onClick={() => setMetric(m.key)} style={{
                      ...
                      color: metric === m.key ? 'white' : C.n600,
                    }}>{m.label}</button>
                  ))}
                </div>
              </div>
            ...
            {/* Summary insight */}
            <div style={{ background: changes.revenue >= 0 ? '#F0FDF4' : '#FEF2F2', borderRadius: 14, padding: '14px 16px', border: `1px solid ${changes.revenue >= 0 ? '#BBF7D0' : '#FECACA'}` }}>
              <div style={{ ...F, fontSize: 13, fontWeight: 700, color: changes.revenue >= 0 ? '#166534' : '#991B1B', marginBottom: 4 }}>
                {changes.revenue >= 0 ? '📈 Performa Meningkat' : '📉 Performa Menurun'}
              </div>
              <div style={{ ...F, fontSize: 12, color: changes.revenue >= 0 ? '#15803D' : '#B91C1C', lineHeight: 1.6 }}>
