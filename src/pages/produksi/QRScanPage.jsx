...
export default function ProduksiQRScanPage({ navigate, goBack }) {
  ...
  const [scannedSequence, setScannedSequence] = useState(null); // item sequence from QR
  ...
  // ── Lookup transaksi ───────────────────────────────────────────────────────
  const lookupTransaction = useCallback(async (notaNo, sequence) => {
    if (!notaNo?.trim()) return;
    setLoading(true);
    setError('');
    setFoundTx(null);
    setScannedSequence(sequence || null);
    try {
  ...
  const scanFrame = useCallback(() => {
    ...
    if (decoded?.data) {
      ...
      // Support formats:
      // 1. WASC-20250618-001#1  (label QR with sequence)
      // 2. WASC-20250618-001    (transaction number with date)
      // 3. WASC-001             (short transaction number)
      // Extract transaction number — everything before #
      const txPart = raw.split('#')[0].trim();
      // Match transaction number (letters, numbers, dashes)
      const notaMatch = txPart.match(/([A-Z0-9][A-Z0-9-]{3,})/);
      const notaNo = notaMatch ? notaMatch[1] : txPart;
      const sequence = raw.includes('#') ? raw.split('#')[1] : null;
      setResult(notaNo);
      stopCamera();
      lookupTransactionRef.current?.(notaNo, sequence);
      return;
    }
    frameRef.current = requestAnimationFrame(scanFrame);
  }, [stopCamera]);
  ...
  const handleNavigate = () => {
    if (!foundTx) return;
    stopCamera();
    // Pass transaction + scanned item sequence so detail page can highlight the specific item
    navigate('detail_item_produksi', { ...foundTx, _scanSequence: scannedSequence });
  };
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n800, overflow: 'hidden' }}>
      <TopBar
        title="Scan Label Nota"
        onBack={goBack}
        rightAction={mode === 'camera' ? toggleFlash : undefined}
        rightIcon={mode === 'camera'
          ? <svg width="22" height="22" viewBox="0 0 24 24" fill={flashOn ? C.warning : 'none'} stroke={flashOn ? C.warning : 'currentColor'} strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
          : undefined
        }
      />

      {/* Mode tabs */}
      <div style={{ display: 'flex', background: C.n800, padding: '6px 16px', gap: 8, flexShrink: 0 }}>
      ...
      {mode === 'camera' && (
        <div style={{ flex: 1, position: 'relative', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
              ...
              <div style={{ width: isMobile ? 200 : 240, height: isMobile ? 200 : 240, position: 'relative' }}>
                ...
                {[
                  ...
                  { top: 0, right: 0, borderTop: `3px solid ${C.info}`, borderRight: `3px solid ${C.info}`, borderRadius: '0 4px 0 0' },
          ...
          {/* Camera error */}
          {cameraError && (
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, background: C.n800 }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>📷</div>
              <div style={{ fontFamily: 'Poppins', fontSize: 14, color: C.n400, textAlign: 'center', marginBottom: 20 }}>{cameraError}</div>
      ...
      {mode === 'manual' && !foundTx && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: isMobile ? 16 : 24, gap: isMobile ? 12 : 16 }}>
          ...
          />
          {error && <div style={{ fontFamily: 'Poppins', fontSize: 12, color: C.danger }}>⚠️ {error}</div>}
      ...
      {(result || foundTx || (error && mode === 'camera')) && (
        <>
          {/* Backdrop */}
          {mode === 'camera' && <div onClick={handleRetry} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 199 }} />}
          {/* Sheet */}
          <div style={{ position: mode === 'camera' ? 'absolute' : 'relative', bottom: 0, left: 0, right: 0, background: C.n800, borderRadius: mode === 'camera' ? '20px 20px 0 0' : 0, padding: `${isMobile ? 16 : 20}px ${isMobile ? 16 : 20}px ${isMobile ? 24 : 32}px`, flexShrink: 0, zIndex: 200 }}>
            {loading && (
            ...
            {foundTx && !loading && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                  ...
                  {scannedSequence && (
                    <span style={{ background: C.info, padding: '2px 8px', borderRadius: 999, fontFamily: 'Poppins', fontSize: 10, fontWeight: 600, color: 'white' }}>
                      Item {scannedSequence} discan
                    </span>
                  )}
                </div>
