...
//   - Midtrans: redirect ke QrPaymentPage
...
import PaymentMethodGrouped, { isMidtransMethod } from '../../components/PaymentMethodGrouped';
...
export default function PelunasanPage({ navigate, goBack, screenParams }) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // Midtrans channel → redirect ke QrPaymentPage (replace history supaya
      // back tidak balik ke pelunasan yang sudah jalan)
      if (isMidtransMethod(selectedMethod)) {
      ...
      // Non-midtrans → recordTransactionPayment langsung
  ...
  if (loading) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50 }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Poppins', color: C.n500 }}>
    ...
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50 }}>
        ...
        <div style={{ padding: 30, textAlign: 'center', fontFamily: 'Poppins' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: C.n900, marginBottom: 6 }}>Tagihan sudah lunas</div>
          <div style={{ fontSize: 13, color: C.n600, marginBottom: 20 }}>
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: C.n50, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px 24px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 14, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.2)' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontFamily: 'Poppins', fontSize: 14, fontWeight: 700 }}>{rp(total)}</div>
            </div>
            {paid > 0 && (
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'Poppins', fontSize: 9, opacity: 0.8 }}>Sudah dibayar</div>
                <div style={{ fontFamily: 'Poppins', fontSize: 14, fontWeight: 700 }}>{rp(paid)}</div>
              </div>
            )}
          </div>
          {tx.customerName && (
            <div style={{ fontFamily: 'Poppins', fontSize: 11, marginTop: 12, opacity: 0.9 }}>
        ...
        <div style={{ background: 'white', borderRadius: 14, padding: '14px 16px', marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.05)' }}>
          <div style={{ fontFamily: 'Poppins', fontSize: 11, fontWeight: 700, color: C.n600, marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.4 }}>
        ...
        {selectedMethod === 'cash' && (
          <div style={{ background: 'white', borderRadius: 14, padding: '14px 16px', marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.05)' }}>
            ...
            {Number(cashReceived) >= balanceDue && (
              <div style={{
                background: '#DCFCE7', borderRadius: 8, padding: '10px 12px',
                fontFamily: 'Poppins', fontSize: 13, color: '#15803D', fontWeight: 700,
            ...
            {Number(cashReceived) > 0 && Number(cashReceived) < balanceDue && (
              <div style={{
                ...
                ⚠️ Kurang {rp(balanceDue - Number(cashReceived))}. Tagihan harus dibayar lunas.
              </div>
            )}
          </div>
        )}
        ...
        {selectedMethod === 'transfer' && (
          <div style={{ background: 'white', borderRadius: 14, padding: '14px 16px', marginBottom: 14, boxShadow: '0 2px 8px rgba(15,23,42,0.05)' }}>
            ...
            <div style={{
              ...
              background: '#FEF9C3', padding: '8px 10px', borderRadius: 6,
        ...
        {selectedMethod === 'deposit' && depositBalance < balanceDue && (
          <div style={{ background: '#FEE2E2', borderRadius: 12, padding: 14, marginBottom: 14, fontFamily: 'Poppins', fontSize: 11, color: '#991B1B' }}>
            ❌ Saldo deposit tidak cukup. Saldo: {rp(depositBalance)}, butuh: {rp(balanceDue)}.
          </div>
        )}

        {isMidtransMethod(selectedMethod) && (
          <div style={{
            background: '#EFF6FF', borderRadius: 12, padding: '12px 14px', marginBottom: 14,
            fontFamily: 'Poppins', fontSize: 11, color: '#1E40AF', lineHeight: 1.5,
            border: '1px solid #BFDBFE',
          }}>
            ⚡ Setelah klik tombol bayar, customer scan QR / buka aplikasi sesuai metode.
            Status update otomatis kalau sudah lunas.
          </div>
        )}
